...
CDist_fun(MSMRWeightTemp_Unfiltered, Whole_data) # whole data so its not filtered

CDist_fun(CortWeightTemp_Unfiltered, Data)

CDist_fun(CortMSMR_Unfiltered, Data)
...
(CortWeight_Plot <- ggplot(CortWeightTemp_Data, aes(x=log(Weight), y = log(Cort))) +
    geom_point(aes(colour = Species, shape = as.factor(Temperature))) +
...
save_plot(plot_grid(CortTemp_Plot, CortWeight_Plot, 
                    labels = c("A", "B"), label_fontface = "plain"),
          filename = paste("Figures/", directory, "/Figure_2.png", sep = ""),
...
save_plot(plot_grid(MSMRTemp_Plot, MSMRWeight_Plot, 
                    labels = c("A", "B"), label_fontface = "plain"),
          filename = paste("Figures/", directory, "/Figure_3.png", sep = ""),
          base_height = 3.3, base_width = 6.6)
...
Stats_Tab <- rbind(coefficients(summary(CortMSMR_Model)),
                   coefficients(summary(CortWeightTemp_Model)),
                   coefficients(summary(MSMRWeightTemp_Model))) %>%
  as.data.frame(.) %>%
  slice(-c(1,3,4,5,6,9,10,11,12,15,16,17)) %>%
  select(., -"t value") %>%
  cbind(., partial_r2 = c(suppressWarnings(rsq.partial(CortMSMR_Model)$partial.rsq[1]),
                          suppressWarnings(rsq.partial(CortWeightTemp_Model)$partial.rsq[1:2]),
                          suppressWarnings(rsq.partial(MSMRWeightTemp_Model)$partial.rsq[1:2])),
        model_r2 = c(summary(CortMSMR_Model)$r.squared, 
                     summary(CortWeightTemp_Model)$r.squared, NA,
                     summary(MSMRWeightTemp_Model)$r.squared, NA),
        n = c(nobs(CortMSMR_Model),
              nobs(CortWeightTemp_Model), NA,
              nobs(MSMRWeightTemp_Model), NA)) %>%
  mutate(across(c(1:5), \(x) round(x, digits = 2))) %>%
  `colnames<-`(c("Est. [95% CI]", "S.E.", "p val.", "Partial R2", "Model R2", "n")) %>%
  mutate(`Est. [95% CI]` = paste0(`Est. [95% CI]`, " [", round(ci_list[,1], 2), ", ", round(ci_list[,2], 2), "]"),
         `p val.` = ifelse(`p val.` < 0.001, "< 0.001", `p val.`)) %>%
  add_row(.before = 1) %>%
  add_row(.before = 3) %>%
  add_row(.before = 6) %>%
  mutate(` ` = c("Cort", "~ MSMR", "Cort", "~ Weight", "~ Temp", "MSMR", "~ Weight", "~ Temp")) %>%
  select(` `, `Est. [95% CI]`, n, `S.E.`, `p val.`, `Partial R2`, `Model R2`) %>%
  `rownames<-`(NULL)

Stats_Tab[is.na(Stats_Tab)] <- ' '
